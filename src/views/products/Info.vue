      ...
      <ul class="list">
        <template v-for="(item, index) in list">
          <li @click.stop="toInfo(item)">
            <div class="icon" :style="{backgroundImage: `url(${require('@/assets/images/icon_folder.png')})`}">
...
<script>
import {collectPamphlet, pamphletInfo, pamphletList, pamphletPage} from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    toInfo({id}) {
      this.$router.push('/products_info1/' + id)
    },
    toSearch() {
      this.limitData.page = 1;
      this.isAll = false;
